...
- **Contract Parsing System**: Parse-once-on-upload architecture extracting and storing structured contract data. Pipeline includes PDF extraction, text normalization, logical part detection, intelligent chunking, Claude Sonnet 4 summarization (clause summaries, defined terms, cross-references, risks), and robust error handling with transactional cleanup. Progress tracking and real-time status updates are provided.
